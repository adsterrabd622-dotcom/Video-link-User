    ...
    <div className="fixed inset-0 bg-slate-950 z-50 flex flex-col font-sans overflow-y-auto">
      ...
      <nav className="relative z-10 p-4 md:p-6 flex justify-between items-center bg-gradient-to-b from-slate-950 to-transparent shrink-0">
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          <div className="flex items-center gap-1.5 md:gap-2 text-[10px] md:text-xs font-medium text-emerald-400 bg-emerald-400/10 px-2 md:px-3 py-2 rounded-full border border-emerald-400/20 backdrop-blur-md text-nowrap">
        ...
        <div className="w-full max-w-md bg-slate-900/80 backdrop-blur-xl border border-white/10 rounded-3xl overflow-hidden shadow-2xl text-center transform transition-all duration-500 relative">
          ...
          <div className="p-6 md:p-8 pt-6">
            ...
            <p className="text-slate-400 mb-6 text-sm leading-relaxed">
                Unlock required. Please watch {TOTAL_ADS} ads to reveal the target link.
            </p>
            ...
            {loadingStatus === 'Loading Ad...' ? (
               <button disabled className="w-full bg-indigo-600/50 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2">
                  Loading Ad...
               </button>
            ) : cooldown > 0 ? (
               <button disabled className="w-full bg-slate-800 border border-slate-700 text-slate-400 font-bold py-4 rounded-xl flex items-center justify-center gap-2">
               ...
               <button disabled className="w-full bg-emerald-600 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 animate-pulse">
                ...
                <button 
                  ...
                  className="w-full bg-white text-slate-900 font-bold py-4 rounded-xl hover:bg-slate-200 transition-all flex items-center justify-center gap-2 shadow-lg"
                >
                    <PlayCircle className="fill-current w-5 h-5"/> Watch Ad to Start
                </button>
            )}
          </div>
        </div>
